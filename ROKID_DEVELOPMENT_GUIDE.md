...
### 5.1 核心里念
- 用户开始/结束控制 + 系统按间隔自动监测（默认5分钟）
- **基线列表模式**：保持食材名称一致性，避免 VLM 每次识别结果不同
- **容错逻辑**：自动检测拍摄问题，防止误统计

### 5.2 前端状态管理（关键）

```typescript
interface MealSessionState {
  sessionId: string;
  status: 'active' | 'ended';
  ...
  baselineNutrition: Nutrition;  // 初始总热量
  
  // 上次有效识别（用于容错回退）
  lastValidFoods: BaselineFood[];
  lastValidNutrition: Nutrition;
  
  // 当前剩余（最新识别结果）
  currentFoods: BaselineFood[];
  currentNutrition: Nutrition;
  
  // 统计
  totalConsumed: Nutrition;  // = baseline - current
}
```
...
┌────────────────────────────────────────────────────────────────┐
│ 1. 开始进餐                                                     │
│    POST /api/v1/vision/analyze {mode: "start"}                 │
│    ↓                                                           │
│    保存 baselineFoods = result.raw_llm.foods                   │
│    保存 lastValidFoods = baselineFoods                         │
│    保存 baselineNutrition = result.snapshot.nutrition          │
│    ↓                                                           │
│    POST /api/v1/meal/start {snapshot}                          │
├────────────────────────────────────────────────────────────────┤
│ 2. 自动拍照更新（每5分钟，推荐使用容错 API）                      │
│    POST /api/v1/vision/analyze_meal_update                     │
│    {image_url, baseline_foods: baselineFoods}                  │
│    ↓                                                           │
│    根据 status 处理：                                           │
│    ├─ "accept" → 更新 lastValidFoods 和 currentFoods           │
│    ├─ "skip"   → 保持 lastValidFoods 不变，静默跳过             │
│    └─ "adjust" → 处理加菜：更新 baselineFoods                   │
│    ↓                                                           │
│    计算 totalConsumed = baselineNutrition - currentNutrition   │
│    ↓                                                           │
│    POST /api/v1/meal/update {session_id, snapshot}             │
├────────────────────────────────────────────────────────────────┤
│ 3. 结束进餐                                                     │
│    POST /api/v1/meal/end {session_id, final_snapshot}          │
│    ↓                                                           │
│    返回最终统计报告                                              │
└────────────────────────────────────────────────────────────────┘
```
...
| 场景 | 检测方式 | 处理 |
|------|----------|------|
| 没拍到食物 | `is_food=false` 或 API 返回 400 | `status=skip`，使用上次数据 |
| 只拍到部分 | 超过50%食材消失 | `status=skip`，使用上次数据 |
| 加菜 | 出现新食材或某食材增加>50% | `status=adjust`，更新基线 |
| 正常消耗 | 食材减少 | `status=accept`，正常更新 |
...
if (comparison.adjustments.length > 0) {
  for (const adj of comparison.adjustments) {
    if (adj.action === "add_new") {
      // 新增食材到基线
      baselineFoods.push({
        dish_name: "新增菜品",
        ingredients: [{name_en: adj.ingredient, weight_g: adj.weight}]
      });
      // 重新计算基线热量
      baselineNutrition.calories += calculateCalories(adj.ingredient, adj.weight);
    }
  }
}
```
...
# 视觉分析
POST /api/v1/vision/analyze              # 多模式分析（mode: start/update）
POST /api/v1/vision/analyze_meal_update  # 带容错的会话更新（推荐）

# 用餐会话
POST /api/v1/meal/start             # query: user_id, meal_type, auto_interval; body: SnapshotPayload
POST /api/v1/meal/update            # query: session_id; body: SnapshotPayload
POST /api/v1/meal/end               # query: session_id; body: SnapshotPayload(可选)
...
| `mode` | string | 否 | `start`(默认) / `update` |
| `baseline_foods` | array | 否 | 基线食物列表（update 模式必填） |
| `reference_image_url` | string | 否 | 参考图片（双图对比模式） |

### analyze_meal_update 请求参数

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `image_url` | string | ✅ | 当前图片 URL |
| `baseline_foods` | array | ✅ | 基线食物列表 |
