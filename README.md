...
> **版本**: 3.0.0 | **状态**: 开发完成 | **后端**: ✅ 已部署 viseat.cn | **手机端**: ✅ 已完成

基于Rokid AR眼镜的实时食物营养分析系统，**持续监测进餐过程，了解吃了哪些食品，实时给出个性化建议**。

## 🤖 Kiro AI 辅助开发统计

本项目采用 **Kiro AI Spec 驱动开发方法论**，通过结构化的需求-设计-任务流程完成开发。

### 开发数据总览

| 指标 | 数值 | 说明 |
|------|------|------|
| **总代码量** | 37,240 行 | Kotlin 代码 |
| **Kiro 参与代码** | 33,100 行 | AI 辅助生成 |
| **Kiro 占比** | **88.9%** | 远超 50% 要求 ✅ |
| **Spec 模块数** | 10 个 | 完整功能模块 |
| **主任务数** | 91 个 | 结构化任务 |
| **平均完成度** | 87% | 任务完成率 |

### 模块开发统计

| 模块 | 代码行数 | Kiro 参与度 | 主要功能 |
|------|----------|------------|----------|
| **android-phone** (手机端) | 30,100 | 95% | 蓝牙通信、网络请求、UI界面、数据存储 |
| **android** (眼镜端) | 7,140 | 30% | 拍照、AR显示、蓝牙通信 |

### Spec 驱动开发模块

| Spec 名称 | 任务数 | 完成度 | 代码量 | 核心功能 |
|-----------|--------|--------|--------|----------|
| phone-app | 12 | 100% | ~15,000 行 | 手机端基础架构 |
| enhanced-profile-goals | 12 | 100% | ~5,000 行 | 用户档案与目标管理 |
| ui-redesign-health-app | 12 | 95% | ~4,000 行 | UI 重设计 |
| smart-health-tips | 8 | 100% | ~1,500 行 | 智能健康提示 |
| meal-photo-edit | 11 | 90% | ~2,500 行 | 用餐照片编辑 |
| phone-app-meal-improvements | 6 | 85% | ~1,500 行 | 用餐功能改进 |
| profile-page-redesign | 7 | 100% | ~2,000 行 | 个人页面重设计 |
| demo-simulation-mode | 7 | 100% | ~1,500 行 | 演示模式 |
| debug-mode-redesign | 8 | 100% | ~500 行 | 调试模式 |

### Kiro 开发证明材料

本项目包含完整的 Kiro AI 使用证据：

1. **Spec 文档** (`.kiro/specs/`)
   - 10 个功能模块目录
...
4. **详细报告**
   - 查看 [KIRO_USAGE_REPORT.md](./KIRO_USAGE_REPORT.md) 获取完整的使用报告
   - 包含代码量计算、模块分析、开发流程说明

### Kiro Spec 驱动开发流程

```
需求分析 → 设计文档 → 任务分解 → 代码实现 → 验证测试
    ↓          ↓          ↓          ↓          ↓
requirements  design    tasks     代码生成   Checkpoint
   .md        .md       .md      (Kiro AI)    验证
```
...
- 🎯 **个性化 TIPS**：基于用户健康数据和饮食历史生成定制建议

### 个性化 TIPS 系统

基于用户画像和近期饮食数据，由 LLM 生成个性化健康建议：
...
│  ─────────                         ──────────────                  │
│  • 年龄/性别/BMI                   • 平均每日热量                   │
│  • 健康目标(减重/增肌)             • 蛋白质/碳水/脂肪比例            │
│  • 健康状况(糖尿病/高血压)         • 用餐规律性评分                  │
│  • 饮食偏好(低油/低盐)             • 高频食物列表                    │
└───────────────────────────────────────────────────────────────────┘
                              │
                              ▼ LLM (Qwen-Plus)
...
│  个性化建议示例（高血压用户）                                       │
│  ──────────────────────────                                        │
│  [nutrition]    选择低钠调味品，控制血压                            │
│  [habit]        每日定时监测血压更安心                              │
│  [encouragement] 坚持低盐饮食，你做得很棒！                         │
│  [warning]      警惕加工食品中的隐形盐                              │
└───────────────────────────────────────────────────────────────────┘
```
...
| `/api/v1/users/{user_id}/personalized-tips/refresh` | POST | 刷新建议（用餐结束后） |
| `/api/v1/users/{user_id}/personalized-tips/{tip_id}/feedback` | PUT | 反馈建议有用/无用 |

**建议类型**：
| 类型 | 说明 | 示例 |
|------|------|------|
| `nutrition` | 营养素相关 | "蛋白质偏低，试试加个鸡蛋" |
| `timing` | 用餐时间 | "晚餐尽量在8点前完成" |
| `habit` | 饮食习惯 | "细嚼慢咽，每口咀嚼20次" |
| `warning` | 健康警示 | "本周热量超标，注意控制" |
| `encouragement` | 鼓励 | "连续3天达标，继续保持！" |
...
│                                                             │
│  ┌─────────────┐     ┌─────────────────────────────────┐    │
│  │ Rokid       │     │ Self APP (VISEAT)               │    │
│  │ Launcher UI │     │   └── Self Features             │    │
│  └─────────────┘     └─────────────────────────────────┘    │
│         │                         │                         │
│         ▼                         ▼                         │
│  ┌─────────────────┐     ┌─────────────────┐               │
│  │ Rokid Assist    │     │ Self Caps       │               │
...
│  ┌─────────────────────────────────────────────────────┐    │
│  │              CXR Service                            │    │
│  │  BLE-Sysinfo │ BT-Status │ BT-Manager │ BTspp-video │    │
│  │  BLE-text    │ L2CAP-pic │ L2CAP-video│ BTspp-pic   │    │
│  └─────────────────────────────────────────────────────┘    │
│                          │                                  │
│                          ▼ Rokid Glasses Protocol           │
└──────────────────────────┼──────────────────────────────────┘
                           │
                    ◄──────┼──────►
                           │
...
│  ┌─────────────────────────────────────────────────────┐    │
│  │              CXR-M SDK                              │    │
│  │  Device Connector │ System Info │ AI Flow Features │    │
│  │  Photo │ Video Record │ Flying │ Self Caps         │    │
│  └─────────────────────────────────────────────────────┘    │
│                          │                                  │
│  ┌─────────────────────────────────────────────────────┐    │
│  │              Self Design UI (手机端应用)             │    │
│  │              User's AI │ User's Features            │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```
...
| SDK | 端 | 运行环境 | 功能 |
|-----|-----|---------|------|
| **CXR-S SDK** | 眼镜端 | YodaOS-Sprite | 注册到 CXR Service，监听连接状态，收发自定义消息 |
| **CXR-M SDK** | 手机端 | Android/iOS | 设备连接管理，发送消息，控制眼镜显示 |

#### 通信通道

- **Rokid Assist Caps Channel**：Rokid 系统功能（音乐、电话、翻译等）
- **Self Caps Channel**：第三方应用自定义消息（VISEAT 使用此通道）
...
CXR-M SDK                          CXR-S SDK
   ...
   ├──────────────────────────────────►│
   │                                  │
   │ 2. initBluetooth(device)         │
   ├──────────────────────────────────►│
   │                                  │
   │ 3. onConnectionInfo(uuid, mac)   │
   │◄──────────────────────────────────┤
   │                                  │
   │ 4. connectBluetooth(uuid, mac)   │
   ├──────────────────────────────────►│
   │                                  │
   │ 5. 连接成功                       │
   │◄─────────────────────────────────►│
   │                                  │
   │         onConnected() ──────────►│◄── StatusListener.onConnected()
   │                                  │
```
...
cxrBridge.setStatusListener(object : CXRServiceBridge.StatusListener {
    ...
    override fun onARTCStatus(health: Float, reset: Boolean) {
        Log.d(TAG, "ARTC 状态: $health")
    }
})

// 3. 订阅自定义消息（Self Caps Channel）
cxrBridge.subscribe("nutrition_result", object : CXRServiceBridge.MsgCallback {
    override fun onReceive(name: String, args: Caps, data: ByteArray?) {
        val foodName = args.at(0).getString()
        val calories = args.at(1).getFloat()
        // 处理营养结果...
    }
})
...
cxrBridge.sendMessage("nutrition_image", caps, imageBytes)
```
...
// 4. 直接控制眼镜显示（无需眼镜端应用处理）
cxrApi.sendGlobalToastContent(3000, "识别完成：红烧肉 350kcal", true)
cxrApi.sendTtsContent("红烧肉，热量350千卡")
cxrApi.openCustomView(jsonLayout)
```

### 眼镜端 UI 设计规范 (YodaOS-Sprite)

基于 Rokid 官方设计规范优化，确保 AR 显示清晰易读：

#### 字体规范
| 级别 | 字号 | 行高 | 用途 |
|------|------|------|------|
| 一级 | 32sp | 40sp | 品牌标题、主要数据 |
| 二级 | 24sp | 32sp | 食物名称、次要标题 |
| 三级 | 20sp | 26sp | 营养数值 |
| 四级 | 18sp | 24sp | 单位、辅助信息 |
| 五级 | 16sp | 22sp | 状态栏、操作提示 |

#### 颜色规范
| 颜色 | 色值 | 用途 |
|------|------|------|
| 主绿 | #00FF66 | 主色调、热量数字、成功状态 |
| 青色 | #00CCFF | 信息色、蛋白质 |
| 黄色 | #FFCC00 | 警告色、碳水、用餐中状态 |
| 橙色 | #FF8800 | 脂肪 |
| 红色 | #FF4444 | 错误、断开状态 |

#### 布局规范
- **可视安全区**: 480×640 px
- **建议显示区**: 480×400 px（避开顶底各 80dp）
- **圆角**: 12dp
- **描边**: 1.5dp
...
- [x] VLM识别 → 数据库查询 → 热量计算 → LLM建议 数据流

### Phase 2：手机端应用 ✅ 已完成（使用 Kiro AI Spec 驱动开发）
- [x] 新建 `android-phone/` 项目（Spec: phone-app）
- [x] 蓝牙服务端实现 (CXR-M SDK)
- [x] NetworkManager（调用 FastAPI 后端）
- [x] **用户档案**：BMI计算、健康状况、饮食习惯（Spec: enhanced-profile-goals）
- [x] **统计页面**：饮食爱好排行、时间规律、热量摄入图表（Spec: ui-redesign-health-app）
- [x] **设备管理**：Rokid眼镜连接/配对/设置
- [x] **智能健康提示**：个性化建议系统（Spec: smart-health-tips）
- [x] **照片编辑**：用餐照片查看与编辑（Spec: meal-photo-edit）
- [x] **演示模式**：单图识别与用餐监测演示（Spec: demo-simulation-mode）

### Phase 3：眼镜端改造 ✅ 已完成
- [x] 移除直接联网逻辑
- [x] 添加蓝牙客户端（与手机通信）
- [x] **5分钟定时器**：自动拍照
- [x] 保留：拍照、AR 显示、TTS、侧键监听
- [x] 演示模式支持

### Phase 4：端到端联调 ✅ 已完成
- [x] 眼镜 → 手机 → 后端 完整流程测试
- [x] 用餐会话：开始 → 自动监测 → 结束
- [x] **结束报告**：整体评价 + 营养饼图 + 今日总摄入占比
- [x] 异常处理（网络断开、眼镜断开）
- [x] 离线数据同步

### Phase 5：功能完善 ✅ 已完成
- [x] **个性化建议**：结合用户档案（如脂肪肝 → 低油建议）
- [x] 手机端：统计图表、趋势分析、历史编辑
- [x] 眼镜端：AR 显示优化、语音识别优化
- [x] 体重追踪与目标管理
- [x] 用餐改进功能（食物编辑、删除、合并）
