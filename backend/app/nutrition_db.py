...
class NutritionDatabase:
    ...
    def _load_builtin_aliases(self) -> None:
        ...
        aliases = {
            ...
            # 火锅相关 - 使用数据库中实际存在的键名
            "hot pot base": "牛油 (炼)",  # 火锅底料按牛油计算
            "hot pot soup": "牛油 (炼)",
            "火锅底料": "牛油 (炼)",
            "fish ball": "鱼丸",
            "fish balls": "鱼丸",
            "鱼丸": "鱼丸",
            "meatball": "meatball",  # FoodStruct 中有
            "meatballs": "meatball",
            "肉丸": "meatball",
            "beef ball": "meatball",
            "beef balls": "meatball",
            ...
            "scallop": "扇贝",
            "lobster": "龙虾",
            "enoki mushroom": "金针菇（鲜）[智力菇]",
            "enoki mushrooms": "金针菇（鲜）[智力菇]",
            "金针菇": "金针菇（鲜）[智力菇]",
            "shiitake": "香菇（鲜）[香草，冬菇]",
            "mushrooms": "香菇（鲜）[香草，冬菇]",
            "菌类": "香菇（鲜）[香草，冬菇]",
            "vermicelli": "豌豆粉丝",  # 粉丝热量为0，用豌豆粉丝
            "glass noodles": "豌豆粉丝",
            "粉丝": "豌豆粉丝",
            "bean curd": "豆腐（代表值）",
            "soy products": "豆腐（代表值）",
            ...
            "tofu skin": "豆腐皮",
            "seafood": "白米虾[水虾米]",
            "seafood platter": "白米虾[水虾米]",
            "海鲜": "白米虾[水虾米]",
            "海鲜拼盘": "白米虾[水虾米]",
            "shrimp": "白米虾[水虾米]",
            "虾": "白米虾[水虾米]",
            "vegetable platter": "生菜[叶用莴苣]",
            "蔬菜拼盘": "生菜[叶用莴苣]",
            "lettuce": "生菜[叶用莴苣]",
            "生菜": "生菜[叶用莴苣]",
            "watercress": "西洋菜",
    ...
    def lookup(self, name: str, category: str = None) -> Tuple[NutritionData, bool]:
        ...
        for db_key, nutrition in self._db.items():
            # 跳过空键和过短的键
            if not db_key or len(db_key) < 2:
                continue
            # 检查是否匹配
            if key in db_key or db_key in key:
                # 计算匹配分数：完全相等最高，否则根据长度相似度
                if key == db_key:
                    return nutrition, True
                # 匹配分数 = 较短者长度 / 较长者长度
                score = min(len(key), len(db_key)) / max(len(key), len(db_key))
                if score > best_score:
                    best_score = score
                    best_match = nutrition
        
        if best_match and best_score > 0.3:  # 至少30%相似度
