...
| 操作 | 非用餐状态 | 用餐监测中 |
|------|------------|------------|
| **点击（侧键/触控板）** | 拍照识别 | 开始3秒结束倒计时 |
| **倒计时中再次点击** | - | 取消结束用餐 |
| **倒计时结束** | - | 自动拍照并结束用餐 |
| **右滑** | - | 直接结束用餐 |

> **结束用餐交互**: 用餐中点击后会显示"3秒后自动结束用餐，取消请再次点击"，倒计时结束后自动拍摄最后一张照片并上传。

### 📷 图像质量配置

| 参数 | 值 | 说明 |
|------|-----|------|
| **输出分辨率** | 1280×960 | 4:3 比例，适合食物拍摄 |
| **JPEG 质量** | 88% | 质量与大小的最佳平衡 |
| **中心裁剪** | 85% | 聚焦食物，保留上下文 |
| **目标文件大小** | 150-500KB | 保证识别精度 |

> **优化说明**: 相比之前的 648×648 正方形输出，新配置提升了分辨率和清晰度，确保食物占画面 50%+ 以获得更好的识别效果。
